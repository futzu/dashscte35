...
import threefive
from threefive.segmentation import table20


def convert_k(k):
    """
    convert_k changes camel case xml names
...
class DashSCTE35:
    ...
    def _iter_attrs(self, attrs):
        ...
        """
        conv = {convert_k(k): convert_v(v) for k, v in attrs.items()}
        pts_vars = ["pts_time", "pts_adjustment", "duration", "segmentation_duration"]
        conv = {k: (ticks2seconds(v) if k in pts_vars else v) for k, v in conv.items()}
    ...
    def _build_descriptor(self, cue):
        if "SegmentationDescriptor" in self.stuff:
            dscptr = threefive.SegmentationDescriptor()
    ...
    def _build_cue(self):
        ...
        if "Binary" in self.stuff:
            cue = threefive.Cue(self.stuff["Binary"]["binary"])
            cue.decode()
        else:
            cue = threefive.Cue()
